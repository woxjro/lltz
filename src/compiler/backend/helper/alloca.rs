...
pub fn exec_alloca(
    ...
    let instructions = match ty {
        Type::Struct { .. } => {
            exec_aggregate_type_alloca_tmp(ty, ptr, register2stack_ptr, memory_ty2stack_ptr)
        }
...
///```
pub fn exec_aggregate_type_alloca_tmp(
    aggregate_ty: &Type,
    ptr: &Register,
    register2stack_ptr: &HashMap<Register, usize>,
    memory_ty2stack_ptr: &HashMap<BackendType, usize>,
    ...
    for (idx, field) in fields.iter().enumerate() {
        ...
        res.append(&mut exec_struct_field_alloca_tmp(
            idx,
            field,
            1,
            memory_ptr,
...
///!!!!!!!!!!michelson-astへ書き換えが完了するまでの仮の関数．移行完了後には消す!!!!!!!!!!!!11
///Struct型のAllocaを実行する
///Struct { id, fields } 型をallocaし, その領域へのポインタをptrへと格納する命令を生成する
///```llvm
///%ptr = alloca Struct { id, fields };
...
fn exec_struct_field_alloca_tmp(
    idx: usize,
    field: &Type,
    depth: usize,
    memory_ptr: &usize,
    ...
    match field {
        Type::Struct { id: _, fields } => {
            ...
            for (child_field_idx, child_field) in fields.iter().enumerate() {
                ...
                res.append(&mut self::exec_struct_field_alloca_tmp(
                    child_field_idx,
                    child_field,
                    depth + 1,
                    memory_ptr,
