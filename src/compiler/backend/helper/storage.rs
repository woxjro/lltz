...
pub fn alloca_storage_by_value(
    ...
    michelson_instructions
        .push(MInstr::Comment(format!("alloca storage {{")).to_instruction_with_comment());

    //Step 0.(parameter, storage)をスタックの一番下に入れる
    michelson_instructions.push(
        MInstr::DugN(register2stack_ptr.len() + memory_ty2stack_ptr.len()).to_instruction_with_comment(),
    );
    ...
    michelson_instructions.push(
        MInstr::DigN(register2stack_ptr.len() + memory_ty2stack_ptr.len()).to_instruction_with_comment(),
    );
